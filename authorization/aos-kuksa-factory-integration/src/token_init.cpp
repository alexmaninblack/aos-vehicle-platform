...
public:
  std::optional<PinState> Inspect() override {
    ...
    if (!state.regular)
      return state;
    const int fd = ::open(std::string(kPinPath).c_str(),
  ...
  std::optional<std::string> Generate() override {
    ...
    for (unsigned char value : bytes)
      pin.push_back(alphabet[value % (sizeof(alphabet) - 1U)]);
    return pin;
  }
  ...
  bool Publish(std::string_view pin) override {
    if (pin.empty() || pin.size() > 256U)
      return false;
    const int fd =
        ::open(std::string(kPinTemporary).c_str(),
    ...
    while (ok && offset < pin.size()) {
      const ssize_t count =
          ::write(fd, pin.data() + offset, pin.size() - offset);
      if (count <= 0)
        ok = false;
      else
    ...
    if (::close(fd) != 0)
      ok = false;
    if (ok)
      ok = ::rename(std::string(kPinTemporary).c_str(),
                    std::string(kPinPath).c_str()) == 0;
    if (!ok)
      ::unlink(std::string(kPinTemporary).c_str());
    return ok;
  }
};
...
public:
  NativeTokenStore() {
    ...
    ready_ = init_ && finish_ && slots_ && info_ && init_token_ && open_ &&
             login_ && init_pin_ && close_ && init_(nullptr) == kOk;
  }
  ...
  std::optional<TokenState>
  Inspect(std::optional<std::string_view> pin) override {
    if (!ready_)
      return std::nullopt;
    ...
    for (Slot slot : *slots) {
      TokenInfo info{};
      if (info_(slot, &info) != kOk)
        continue;
      std::string label(reinterpret_cast<char *>(info.label),
                        sizeof(info.label));
      while (!label.empty() && label.back() == ' ')
        label.pop_back();
      if (label != kTokenLabel)
        continue;
      ++result.matching_tokens;
      if (pin && LoginUser(slot, *pin))
        result.user_pin_valid = true;
    }
    return result;
  }

  bool Initialize(std::string_view user_pin, std::string_view so_pin) override {
    if (!ready_ || user_pin.empty() || so_pin.empty())
      return false;
    auto slots = Slots(false);
    if (!slots)
      return false;
    std::vector<Pkcs11TokenSlot> states;
    for (Slot slot : *slots) {
      TokenInfo info{};
      const Rv result = info_(slot, &info);
      states.push_back({slot, result, result == kOk ? info.flags : 0UL});
    }
    const auto candidate = SelectSingleUninitializedSlot(states);
    if (!candidate) {
      std::fprintf(stderr,
                   "token-init: stage=select-uninitialized-slot rv=0x0\n");
      return false;
    }
    ...
    Rv result =
        init_token_(*candidate, Bytes(so_pin), so_pin.size(), label.data());
    if (result != kOk) {
      std::fprintf(stderr, "token-init: stage=init-token rv=0x%lx\n", result);
      return false;
    }
    Session session = 0;
    result = open_(*candidate, kSerialSession | kRwSession, nullptr, nullptr,
                   &session);
    if (result != kOk) {
      std::fprintf(stderr, "token-init: stage=open-session rv=0x%lx\n", result);
      return false;
    }
    result = login_(session, kUserSo, Bytes(so_pin), so_pin.size());
    bool ok = result == kOk;
    if (!ok)
      std::fprintf(stderr, "token-init: stage=login-so rv=0x%lx\n", result);
    if (ok) {
      result = init_pin_(session, Bytes(user_pin), user_pin.size());
      ok = result == kOk;
      if (!ok)
        std::fprintf(stderr, "token-init: stage=init-user-pin rv=0x%lx\n",
                     result);
    }
    if (ok) {
      ...
      close_(session);
      return LoginUser(*candidate, user_pin);
    }
    close_(session);
    return false;
  }
...
private:
  ...
  std::optional<std::vector<Slot>> Slots(bool present) {
    ULong count = 0;
    if (slots_(present ? 1 : 0, nullptr, &count) != kOk || count == 0 ||
        count > 64U) {
      return std::nullopt;
    }
    std::vector<Slot> result(count);
    if (slots_(present ? 1 : 0, result.data(), &count) != kOk)
      return std::nullopt;
    result.resize(count);
    return result;
  }
  bool LoginUser(Slot slot, std::string_view pin) {
    Session session = 0;
    if (open_(slot, kSerialSession | kRwSession, nullptr, nullptr, &session) !=
        kOk)
      return false;
  ...
  InitToken init_token_{nullptr};
  OpenSession open_{nullptr};
  Login login_{nullptr};
  InitPin init_pin_{nullptr};
...
SelectSingleUninitializedSlot(const std::vector<Pkcs11TokenSlot> &slots) {
  std::optional<unsigned long> selected;
  for (const auto &slot : slots) {
    if (slot.result != kOk || (slot.flags & kPkcs11TokenInitialized) != 0U)
      continue;
    if (selected)
      return std::nullopt;
    selected = slot.id;
...
InitResult InitializeToken(PinStore &pins, TokenStore &tokens) {
  ...
  if (!pin)
    return InitResult::kUnavailable;
  if (pin->exists) {
    if (!pin->regular || !pin->root_owned || pin->mode != 0600U ||
        pin->value.empty() || pin->value.size() > 256U)
      return InitResult::kRejected;
    auto token = tokens.Inspect(pin->value);
    if (!token)
      return InitResult::kUnavailable;
    return token->matching_tokens == 1U && token->user_pin_valid
               ? InitResult::kValidated
               : InitResult::kRejected;
  }
  auto token = tokens.Inspect(std::nullopt);
  if (!token)
    return InitResult::kUnavailable;
  if (token->matching_tokens != 0U)
    return InitResult::kRejected;
  auto user_pin = pins.Generate();
  auto so_pin = pins.Generate();
  if (!user_pin || !so_pin || *user_pin == *so_pin)
    return InitResult::kUnavailable;
  if (!tokens.Initialize(*user_pin, *so_pin))
    return InitResult::kUnavailable;
  if (!pins.Publish(*user_pin))
    return InitResult::kUnavailable;
...
int main() {
  ...
  return result == aos::factory::InitResult::kCreated ||
                 result == aos::factory::InitResult::kValidated
             ? 0
             : 1;
}
#endif
